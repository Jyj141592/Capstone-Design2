...
function ExploreClub(){
    ...
    useEffect(()=>{
        setClubs([{id: 1, name: 'c1'}, {id: 2, name: 'c2'}, {id: 3, name: 'c3'},{id:4, name:'c4'}]);
    }, []);
    ...
    return (
        <div className={styles.container}>
            <div className={styles.topBar}>
                <select className={styles.filterSelect} onChange={(e)=>onSelection(e)}>
                    <option value="name">이름</option>
                    <option value="interest">관심사</option>
                </select>
                <input type="text" className={styles.searchInput} placeholder="모임 검색" onChange={(e)=>onKeyword(e)}/>
                <button className={styles.searchButton} onClick={handleSearch}>검색</button>
                <button className={styles.createButton} onClick={() => navigate('/club/create')}>모임 생성</button>
            </div>
